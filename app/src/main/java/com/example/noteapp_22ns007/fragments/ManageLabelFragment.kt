    ...
    private fun initializeViewElements(binding: FragmentManageLabelBinding) {
        ...
        binding.addLabelBtn.setOnClickListener {
            ...
            if(newLabelName.isBlank()) {
                Toast.makeText(context, "Nhãn không được trống", Toast.LENGTH_SHORT).show()
            ...
            addLabelSuccessLiveData?.observe(viewLifecycleOwner) { success ->
                if (!success) {
                    Toast.makeText(context, "Nhãn \"$newLabelName\" đã tồn tại", Toast.LENGTH_SHORT).show()
                } else {
                    Toast.makeText(context, "Đã thêm nhãn \"$newLabelName\"", Toast.LENGTH_SHORT).show()
